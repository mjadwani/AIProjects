...
# Create the Tool
# db2_help_tool = create_retriever_tool(
#     vectorstore.as_retriever(search_kwargs={"k": 3}),
#     "search_db2_manuals",
#     "Use this tool to look up technical definitions for Db2 ZPARMS, "
#     "error codes, and best practices from the manuals. This knowledge is from Db2 V13 manual."
# )



# Add to your existing agent's tool list
# tools = [getzparm_tool, db2_help_tool]


# This would replace your ChatOllama call
# It requires the model to be exported to OpenVINO format
# model = OpenVINO(
#     model_id="intel/gemma-4-e4b-openvino", 
#     device="NPU",  # This forces it to the AI Boost chip
...
def authidtool():
    ''' 
    this is TROTEL service 
    this service returns db2 authid who is making the call
     to this service
       '''
    return apicall.authid()

@tool
def clientaccttool():
    '''  this is TROTEL service 
     this service returns db2 client which is used to
     make this service call       '''
    return apicall.workstation()

@tool
def fullresponsetool():
    ''' this is TROTEL service 
        this service returns complete response received 
        when this service is called
                      '''
    
    return apicall.fullresponse()
    ...
    for chunk in servicenode.stream(init_state,config=config):
        ...
        for node_name, state_update in chunk.items():
            ...
            if "messages" in state_update:
                ...
                # if isinstance(last_msg, dict) and 'raw' in last_msg:
                #     print(f"""Content: {"x",last_msg['raw'].content} 
                #            "Token Usage":{last_msg['raw'].usage_metadata}""")
                # else:
                #     # print(last_msg)
                #     print(f"Content: {last_msg.content} ")
                #     # print(f"Token Usage:{last_msg.usage_metadata}")
